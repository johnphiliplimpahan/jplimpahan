      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-zinc-800 border border-zinc-800">
          {skillCategories.map((category, index) => (
            <div key={index} className="bg-[#050505] p-8 group">
            ...
            {[...Array(2)].map((_, i) => (
              <div key={i} className="flex gap-12 items-center px-6">
                {skillCategories.flatMap(c => c.skills).map((skill, idx) => (
                  <span key={idx} className="text-4xl md:text-6xl font-display font-black text-zinc-900 uppercase tracking-tighter hover:text-emerald-500/20 transition-colors duration-500">
                    {skill}
                  </span>
                ))}
              </div>
